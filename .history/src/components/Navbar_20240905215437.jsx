...
const Navbar = () => {
  ...
  const headerRef = useRef(null); // Definición de headerRef

  const handleScroll = () => {
    const sy = window.scrollY;
    const delta = 10;
    const navHeight = headerRef.current ? headerRef.current.offsetHeight : 70; // Usar headerRef
  ...
  return (
    <header
      ref={headerRef} // Usar headerRef aquí
      id='navbar'
      className={`fixed w-full z-50 border-b border-gray-700/50 py-2 transition-transform duration-300 ${
        navUp ? '-translate-y-full' : 'translate-y-0'
      }`}
      style={{
        background: '#132c57',
